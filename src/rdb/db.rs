...
use crate::{LIST_QUICKLIST_ENCODING, Pair, parse_val_type, Parser, STRING_ENCODING};
    ...
    fn parse(cursor: &mut Cursor<&[u8]>) -> Result<DbInfo> {
        ...
        loop {
            ...
            match flags {
                ...
                _ => {
                    let  val_type = flags;
                    println!("val_type:{:x?}",val_type);
                    //因为flags一直在匹配，所以有可能会错失一些值，这里不需要继续对cursor做解析
                    if is_val_type(cursor) {
...
fn is_val_type(cursor: &mut Cursor<&[u8]>) -> bool {
    cursor.set_position(cursor.position()-1);
